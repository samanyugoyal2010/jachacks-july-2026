...
const BANKS: Bank[] = [
  {
    id: "chase",
    name: "Chase",
    blurb: "Largest US bank by assets",
    profile:
      "Conservative on debt-to-income and prefers an existing deposit relationship. Rewards long job tenure. Rarely the easiest approval, but strong terms when it says yes.",
  },
  {
    id: "bofa",
    name: "Bank of America",
    blurb: "National retail lender",
    profile:
      "Middle-of-the-road on DTI. Weighs verified income heavily and is noticeably friendlier to existing customers. Unverified income is close to an automatic decline.",
  },
  {
    id: "wells",
    name: "Wells Fargo",
    blurb: "Large branch network",
    profile:
      "Relationship-driven and comparatively strict since its consent-order years. Wants documented income and stable employment; thin files struggle.",
  },
  {
    id: "citi",
    name: "Citibank",
    blurb: "Global consumer bank",
    profile:
      "Tighter than average on DTI and focused on higher income bands. Less interested in collateral than in cash flow.",
  },
  {
    id: "usbank",
    name: "U.S. Bank",
    blurb: "Midwest-anchored national bank",
    profile:
      "Moderate and fairly predictable. Gives real weight to appraised collateral, which can offset a borderline DTI more than most peers.",
  },
  {
    id: "pnc",
    name: "PNC Bank",
    blurb: "East-coast regional at national scale",
    profile:
      "Pragmatic mid-market lender. Comfortable with moderate DTI when employment is stable and income is documented.",
  },
  {
    id: "capone",
    name: "Capital One",
    blurb: "Data-driven consumer lender",
    profile:
      "The most tolerant of thinner or imperfect profiles on this list. Prices for risk rather than declining it, so approval odds stay higher across the board.",
  },
  {
    id: "td",
    name: "TD Bank",
    blurb: "East-coast retail bank",
    profile:
      "Straightforward retail underwriting. Wants clean documentation; flexible on collateral, unremarkable on everything else.",
  },
  {
    id: "truist",
    name: "Truist",
    blurb: "BB&T / SunTrust successor",
    profile:
      "Southeast-weighted regional. Middling DTI tolerance and a genuine preference for secured lending over unsecured.",
  },
  {
    id: "amex",
    name: "American Express",
    blurb: "Premium unsecured lender",
    profile:
      "The most selective here. Targets high income and low DTI, lends unsecured so collateral counts for nothing, and effectively requires verified income.",
  },
];
...
function toProfile(facts: Record<string, string>): Profile {
  ...
  return {
    ...
    collateral_appraised:
      truthy(facts.collateral_appraised, false) && ctype !== "none",
  };
}
...
function describe(p: Profile): string {
  ...
  if (p.tenure_years)
    bits.push(`Years at current job: ${Math.round(p.tenure_years)}`);
  bits.push(
...
function fallback(p: Profile): Rated[] {
  ...
  if (!p.income_verified) {
    base = 18;
    why =
      "Income is not documented, which most lenders treat as a blocking gap rather than a weakness.";
  } else {
    const pctDti = Math.round(p.dti * 100);
    if (p.dti <= 0.2) {
      base = 82;
      why = `Debt-to-income of ${pctDti}% is comfortably inside typical limits.`;
    } else if (p.dti <= 0.36) {
      base = 66;
      why = `Debt-to-income of ${pctDti}% sits in the normal approval band.`;
    } else if (p.dti <= 0.43) {
      base = 46;
      why = `Debt-to-income of ${pctDti}% is near the common 43% ceiling.`;
    } else if (p.dti <= 0.55) {
      base = 24;
      why = `Debt-to-income of ${pctDti}% is above the limit most lenders hold to.`;
    } else {
      base = 9;
      why = `Debt-to-income of ${pctDti}% is far above standard limits.`;
    }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (body && typeof body.facts === "object" && body.facts)
      facts = body.facts;
  } catch {
