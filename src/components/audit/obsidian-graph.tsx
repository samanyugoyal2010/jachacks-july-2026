...
export function ObsidianGraph({
  ...
  function computeFit(): Camera | null {
    ...
    for (const n of simNodes) {
      ...
      const padBottom = n.kind === "stage" ? r + 24 : r; // room for the label
      minX = Math.min(minX, n.x - r - (n.kind === "stage" ? 34 : 0));
      maxX = Math.max(maxX, n.x + r + (n.kind === "stage" ? 34 : 0));
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-[400px] w-full overflow-hidden rounded-xl border border-border/60 bg-card/70 shadow-lg shadow-black/5 backdrop-blur-xl backdrop-saturate-150 sm:h-[460px]"
    >
      <svg
        ref={svgRef}
        width="100%"
              ...
              >
                ...
                {n.kind === "stage" && (
                  <text
                    y={r + 16}
                    textAnchor="middle"
                    className="font-display pointer-events-none fill-foreground text-[11px] font-bold"
                  >
                    {n.label}
                  </text>
                )}
